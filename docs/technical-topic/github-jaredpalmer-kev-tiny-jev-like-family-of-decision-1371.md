...
title: 'GitHub - jaredpalmer/kev: tiny Jev-like family of decision models built on
  top of Qwen3.5 you can train and run on your own · GitHub'
domain: github.com
...
- ai
- python
summary: Kev is an open-source family of small decision models (0.8B, 4B, and 9B parameters)
  built on Qwen 3.5 that can handle multiple question types (yes/no, multiple-choice,
  and rating) in a single request, with an API compatible with TypeSafe's System One.
  The models are designed to run locally on consumer hardware including Apple Silicon
  Macs, and include training code, evaluation data, and a web playground for testing.
  Kev-9B achieves strong performance on decision-making tasks with 0.852 accuracy
  on new datasets and includes optional features like temperature scaling and date
  arithmetic to improve calibration and reasoning.
...
[![Frozen eval suites](https://camo.githubusercontent.)](https://huggingface.co/datasets/jaredpalmer/kev-suites)
[![Research log](https://camo.githubusercontent.)](/jaredpalmer/kev/blob/main/PLAN.md)
[![License: Apache-2.0](https://camo.githubusercontent.)](/jaredpalmer/kev/blob/main/LICENSE)
...
[![Kev playground](/jaredpalmer/kev/raw/main/docs/playground.png)](/jaredpalmer/kev/blob/main/docs/playground.png)
...
[![Kev chess](/jaredpalmer/kev/raw/main/docs/chess.png)](/jaredpalmer/kev/blob/main/docs/chess.png)
...
| --- | --- | --- | --- | --- | --- |
| [Kev-0.8B](https://huggingface.co/jaredpalmer/kev-0.8b) | Qwen3.5-0.8B-Base | 0.825 / 0.834 | 0.652 / 0.684 | 0.499 / 0.460 | [Details](/jaredpalmer/kev/blob/main/docs/model-cards/kev-0.8b.md) |
| [Kev-4B](https://huggingface.co/jaredpalmer/kev-4b) | Qwen3.5-4B-Base | 0.872 / 0.871 | 0.797 / 0.837 | 0.299 / 0.255 | [Details](/jaredpalmer/kev/blob/main/docs/model-cards/kev-4b.md) |
| [Kev-9B](https://huggingface.co/jaredpalmer/kev-9b) | Qwen3.5-9B-Base | 0.872 / 0.874 | **0.822 / 0.852** | **0.286 / 0.237** | [Details](/jaredpalmer/kev/blob/main/docs/model-cards/kev-9b.md) |
...
[![Accuracy by source for Kev and Jev](/jaredpalmer/kev/raw/main/docs/kev-family.png)](/jaredpalmer/kev/blob/main/docs/kev-family.png)
...
| Model | Base | Accuracy: Trained Sources | Accuracy: New Sources | Brier: New Sources | Model Card |
| --- | --- | --- | --- | --- | --- |
| Kev-0.6B (Qwen3) — `jaredpalmer/kev-0.6b` | Qwen3-0.6B-Base | 0.801 / 0.808 | 0.620 / 0.642 | 0.536 / 0.483 | [Details](/jaredpalmer/kev/blob/main/docs/model-cards/kev-0.6b-qwen3.md) |
| Kev-4B (Qwen3) — `jaredpalmer/kev-4b@qwen3` | Qwen3-4B-Base | 0.854 / 0.856 | 0.790 / 0.806 | 0.328 / 0.294 | [Details](/jaredpalmer/kev/blob/main/docs/model-cards/kev-4b-qwen3.md) |
| Kev-8B (Qwen3) — `jaredpalmer/kev-8b` | Qwen3-8B-Base | 0.863 / 0.870 | 0.796 / 0.780 | 0.337 / 0.327 | [Details](/jaredpalmer/kev/blob/main/docs/model-cards/kev-8b-qwen3.md) |

Because only the base changed, the two generations are a controlled comparison. On the development set the accuracy gain is within noise; on the test set Kev-9B is 7.3 points ahead of Kev-8B (95% CI +2.8 to +11.7) with a Brier score 0.08 lower, Kev-4B is 2.9 points ahead of its predecessor (−0.9 to +6.4), and Kev-0.8B is 4.8 points ahead of Kev-0.6B (+0.2 to +9.3). [PLAN\_Qwen35.md](/jaredpalmer/kev/blob/main/PLAN_Qwen35.md) has the full experiment, including the criteria we set in advance and how the results measured against them.

The original [Kev-0.5B](https://huggingface.co/jaredpalmer/kev-0.5b) used Qwen2.5-0.5B and is kept for reference; see its [model card](/jaredpalmer/kev/blob/main/docs/model-cards/kev-0.5b.md).
...
Use `uv run python -m kev.train --help` for all training options. The released models don't use the optional `--perm_kl` or `--ord_w` losses. The [model cards](/jaredpalmer/kev/blob/main/docs/model-cards) have the training settings and dataset lists; [PLAN.md](/jaredpalmer/kev/blob/main/PLAN.md) records what was tried and what helped.
...
[Study plans](/jaredpalmer/kev/blob/main/experiments/v7-final.json) list training settings. Each trial saves the settings, code hashes, dataset hashes, and results. Choose models using the development results, not the locked test. After choosing a final candidate, you can read its test results once:
...
`kev.jev` runs the same questions against Jev through Vercel AI Gateway. `kev.compare` compares two saved runs with paired bootstrap confidence intervals. For the full experiment history, see [PLAN.md](/jaredpalmer/kev/blob/main/PLAN.md) and [the leaderboard](/jaredpalmer/kev/blob/main/runs/leaderboard.md).
...
[Apache-2.0](/jaredpalmer/kev/blob/main/LICENSE). The Qwen3 and Qwen3.5 base models are also Apache-2.0. Training datasets have their own licenses; see the [model cards](/jaredpalmer/kev/blob/main/docs/model-cards).
