...
- llm
- security
summary: Modern AI inference APIs are increasingly moving away from portable sessions,
  where users can export their conversation history and continue it with a different
  provider or model. Instead, providers are locking users into their ecosystems through
  opaque encrypted reasoning tokens, hidden web search results, server-side state
  storage, and provider-sealed data that cannot be inspected or transferred. This
  shift fundamentally changes session ownership from users to providers, making it
  impossible to truly own, audit, or port conversations despite providers' privacy
