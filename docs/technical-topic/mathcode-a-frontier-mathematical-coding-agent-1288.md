...
- llm
- cli-tool
summary: MathCode is a terminal-based AI coding assistant that converts natural language
  math problems into formal Lean 4 theorems and automatically attempts to prove them.
  It features a persistent Lean REPL, reusable theorem and axiom libraries, an interactive
  proving agent that breaks complex proofs into subgoals, and an Obsidian knowledge
  graph to visualize theorem dependencies. The tool is available for macOS and Linux
  systems and can be tried with simple commands to formally verify mathematical statements.
...
![MathCode demo](Demo.png)

Quick Start
-----------

Requires macOS (arm64) or Linux (x86\_64), plus the `codex` CLI for the default backend.
...
The math formalization and proving pipeline is based on the [AUTOLEAN](https://github.com/T3S1AMAX/autolean.git) project.
