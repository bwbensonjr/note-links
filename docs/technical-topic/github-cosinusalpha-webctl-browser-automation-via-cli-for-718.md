...
title: 'GitHub - cosinusalpha/webctl: Browser automation via CLI — for humans and
  agents'
domain: github.com
...
- web-dev
- ai
summary: webctl is a command-line tool for browser automation designed for both AI
  agents and humans, built on Playwright and Chromium. Unlike MCP-based alternatives,
  webctl gives users control over what information enters their context by allowing
  filtering and piping of output through Unix tools before processing, avoiding context
  bloat. The tool provides semantic element targeting using ARIA roles, session persistence,
  and straightforward commands for navigation, observation, interaction, and waiting,
  making it suitable for scripting, debugging, or integrating with AI agents like
  Claude.
