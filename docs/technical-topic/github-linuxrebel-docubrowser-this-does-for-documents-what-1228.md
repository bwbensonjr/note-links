...
title: 'GitHub - linuxrebel/DocuBrowser: This does for Documents what repo-browser
  does for repos. A local AI powered Document search engine · GitHub'
domain: github.com
...
- llm
- python
summary: DocuBrowser is a local AI-powered document search engine that transforms
  scattered files (PDFs, Word docs, emails, ebooks, etc.) into a searchable knowledge
  base using semantic understanding rather than just keyword matching. It runs entirely
  on your machine using open-source models like Ollama, requiring no internet connection,
  accounts, or API keys, and includes features like AI-generated summaries, PII detection,
  and support for 40+ document formats. The tool offers both keyword and semantic
  search modes with a web interface, making it easy to find documents by meaning even
  when exact wording doesn't match.
fetch_status: success
summarizer_model: global.anthropic.claude-haiku-4-5-20251001-v1:0
---

# GitHub - linuxrebel/DocuBrowser: This does for Documents what repo-browser does for repos. A local AI powered Document search engine · GitHub
...
* **Platform**: all extraction libraries are pure Python or have wheels for both x86\_64 and ARM64. 32-bit systems are not supported.
...
* Memory-safe: kernel-enforced RLIMIT\_AS (6 GB/worker) + pause/resume on free-RAM threshold
...
| --- | --- |
| [Dark mode](/linuxrebel/DocuBrowser/blob/main/screenshots/screenshot-dark-mode.png) | [Light mode](/linuxrebel/DocuBrowser/blob/main/screenshots/screenshot-light-mode.png) |



| Settings | AI Synopsis |
| --- | --- |
| [Settings page](/linuxrebel/DocuBrowser/blob/main/screenshots/screenshot-settings-page.png) | [Synopsis modal](/linuxrebel/DocuBrowser/blob/main/screenshots/screenshot-synopsis-modal.png) |
...
**Minimum:** 8 GB RAM, x86\_64 or ARM64 CPU, 2 GB free disk (plus space for your documents and the index). Works without a GPU — synopsis generation will be slower but functional. 32-bit systems are not supported (Ollama does not ship 32-bit builds).
...
* Per-column BM25 weights echo the old field priorities
  (name 6, title 8, author 7, subject 5, description 3, content\_snippet 3,
...
| Limitation | Status |
| --- | --- |
| DRM-encrypted AZW not fully searchable | Metadata indexed; DeDRM\_tools required for body text |
| Scanned PDFs not searchable | Listed in ocr\_list\_pdfs.txt; OCR deferred |
...
### No-default doc\_dir, configure banner, uninstall.sh
...
See [LICENSE](/linuxrebel/DocuBrowser/blob/main/LICENSE) or <https://www.gnu.org/licenses/gpl-3.0.html>.
