...
- mathematics
- ai
summary: The article explores the fundamental connection between data compression
  and language modeling, demonstrating that both rely on identifying patterns and
  predicting what comes next. Using arithmetic coding as the primary example, it explains
  how compressors work by building probability models of data—assigning higher probabilities
  to more frequent symbols—and using entropy coders to convert these probabilities
  into efficient binary representations. The key insight is that better probability
  predictions lead to better compression, establishing compression as fundamentally
...
Transforms are the preprocessing steps that make our data easier to compress. The method we saw earlier
(run-length encoding) is an example of a transform, but it’s worth noting that transforms don’t *always* shrink the
data. Sometimes they can be used to create *more* redundancy, and the more redundancy, the more we can compress later
on. We aren’t going to focus on transforms in this article, but they’re still an important part of any compression tool.

Models describe the *shape* of our data based on the frequencies of each **symbol** (whatever unit we’re
using to look for redundancies: letters, numbers, tokens, or even binary code). For now, you can think of a model as a
table that maps each **symbol** to its **probability**, but as we’ll see later on, they can get *a lot* more
sophisticated.
...
Entropy coders are almost always the final step in any compression algorithm and are what produce the final
compressed artifact: a **raw bitstream**, which is just a bare sequence of bits with none of the structure a file format
...
Bookmark this sectionSquishing data with probabilities
------------------------------------------------------
...
### Bookmark this sectionArithmetic coding
...
Nothing encoded yet. Full range 0 to 1. Next, encode A.ABC00.5710.8571

Previous characterABABAACNext character

Range: [0.00000, 1.00000)

Once we run out of symbols, we end up with a *teeny weeny baby range*: **[0.38730, 0.38855)**.

**The mixed brackets are intentional.** Square brackets [ ] mean endpoint *included*, round brackets ( ) mean endpoint
*excluded*. So [0, 1) is “all numbers from 0 to 1, including 0 but excluding 1”.
...
Our final number is *not* a floating point—it’s a **binary fraction**. Floating points are binary fractions too, but
they come in fixed widths, so you’d pay 32 or 64 bits whether you need them or not. Ours only needs 10.
...
### Bookmark this sectionDecompressing arithmetic codes
...
Ready to decode the number 0.3876953125. Full range 0 to 1. Press next to decode the first symbol.ABC00.5710.8571

Previous characterNext character
...
Bookmark this sectionHow probabilities affect compression
---------------------------------------------------------
...
|  | ABABAACA B A B A A C | AAAAAAAAAABCA A A A A A A A A A B C |
| --- | --- | --- |
...
“Wait, isn’t entropy a physics thing?” you might ask. Yes! But what we’re talking about is **Shannon entropy**, which
is related to data compression (in the field of information theory). What’s cool is that its mathematical formula is
nearly identical to the Gibbs formula for entropy in thermodynamics. Wild, huh?

Bookmark this sectionEntropy
----------------------------
...
−log2(probability)negative log base 2 of probability
...
| Symbol | −log2(P)negative log base 2 of P | Bits |
| --- | --- | --- |
| bird | −log2(0.5)negative log base 2 of 0.5 | 1 |
| squirrel | −log2(0.25)negative log base 2 of 0.25 | 2 |
| cat | −log2(0.125)negative log base 2 of 0.125 | 3 |
| fox | −log2(0.0625)negative log base 2 of 0.0625 | 4 |
| bear | −log2(0.0625)negative log base 2 of 0.0625 | 4 |
...
Note: this floor only applies when you don’t want to *lose data*, but compressors like JPEG or MP3 *can* get smaller
by throwing out details that won’t be missed. This is called **lossy compression**. Everything discussed here is about
**lossless** compression, where no data is lost, but both rely on models and probabilities to shrink data.
...
Bookmark this sectionContext matters
------------------------------------
...
Nothing encoded yet. Full range 0 to 1. Next, encode T.TO(space)BERN00.1670.3890.6670.7780.8890.9441

Previous characterTO BE OR NOT TO BENext character
...
Bookmark this sectionLanguage modeling and compression
------------------------------------------------------
...
0.00 bits + −log2(0.00)negative log base 2 of 0.00
...
Bermudathe model’s confident guess−log2(0.82)negative log base 2 of 0.82 = 0.29 bits

Spainthe actual next word−log2(0.02)negative log base 2 of 0.02 = 5.64 bits
...
Bookmark this sectionCompression in the wild
--------------------------------------------
...
Bookmark this sectionTwo sides, same coin
-----------------------------------------
...
* [### Quantization from the ground up](/blog/quantization)
