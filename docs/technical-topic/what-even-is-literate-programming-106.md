...
Knuth means by a “documentation language” means the same thing *you*
...
Look at Knuth’s *TeX: The Program* and *Metafont: The Program*, and
...
we want a computer to do? Kind of… So are they *not* literate programs?

This is one reason why I wrote this blog post. The rest of the post is unchanged.

**Aside:** After thinking about it for a *long* time, I suspect the
...
*Structured Programming* by Dijkstra, Hoare, and friends, to have
...
*as Mills described top-down design*: carve up a program into
...
> *IEEE Trans. on Software Eng.* **SE–5** (1979), 374–386, [doi:10.1109/TSE.1979.234205](https://doi.org/10.1109/TSE.1979.234205)]; this
...
**Addendum** Feb 11, 2025 (8:32:58 AM).
...
* Then do a sort of *quick sketch* adding more named chunks of code,
  outlining the design further in commentary, and each *quick sketch* is
...
David R. Hanson’s *C Interfaces and Implementations: Techniques for Creating Reusable Software*
...
*Retargetable C Compiler, A: Design and Implementation* (Addison-Wesley
Professional, 1995) is another example cited as a literate program, but
it is a *post hoc* literate program: the C compiler was written first,
...
2. Avoid large-scale cyclic dependencies, minimize “forward declarations”
   of concepts. I took this to mean that ideally in chapter *N*, you
...
[retrocomputing project ideas](/2024/05/26/retrocomputing-projects.html),
...
you see this in *Crafting Interpreters* where Nystrom refactors portions
...
explain what they’re doing is hard (because *giving explanations* is hard).
...
to a literate program were seen by Knuth as analogous to *errata* for a
book (there are “changefiles” for WEB programs which are modeled after
*errata*), which is farthest from the contemporary usage.
...
*How do I write a Knuth-style literate program?*

This is hard, because it boils down to
*How do I write well in computer science?*
...
*just for sorting*. And we’re assuming that the design space is well
...
The difficult that crops up is there are so few *well thought out* tools for literate
programming that you may have to roll your own. (Dare I say, you could
write a *literate program* for literate programming?)
...
the battle for writing *in general*. Paul Halmost once remarked (as
dutifully noted in Knuth and friends’s [*Mathematical Writing*](https://jmlr.csail.mit.edu/reviewing-papers/knuth_mathematical_writing.pdf)) that the
...
I think this is what Knuth does in his *Art of Computer Programming*.
...
*Art of Computer Programming* limits itself to a depth of 4, why Alan
...
I believe Bob Nystrom wrote the code for *Crafting Interpreters*
hand-in-hand with the outline of the book, revised the code and outline
iteratively, and once the code was finished *only then* did he start
...
The key step in writing is *rewriting* (arguably this is true for
...
*post hoc* explanations (like Hanson and Fraser published) or
...
The difficulty is that you are writing *for a reader*, not for your own
edification. What clearly communicates the concepts as well as the code?
Could *you* reproduce your code in a different language after reading
...
*Art of Computer Programming*, when he uses MIXAL it is exemplary
