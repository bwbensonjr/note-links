---
id: 79
url: https://computerhistory.org/blog/adobe-photoshop-source-code/
title: Adobe Photoshop Source Code - CHM
domain: computerhistory.org
source_date: '2025-12-23'
tags:
- github-repo
- c
- academic-paper
summary: The Computer History Museum acquired and published the source code for Adobe
  Photoshop 1.0, marking a significant preservation effort of computing history. This
  acquisition allows researchers and developers to study the original code that launched
  one of the most influential image editing software programs ever created. The preservation
  highlights the importance of archiving foundational software to understand the evolution
  of digital technology.
fetch_status: success
summarizer_model: global.anthropic.claude-haiku-4-5-20251001-v1:0
---

# Adobe Photoshop Source Code - CHM

[Home](https://live-comp-history-museum.pantheonsite.io/)  [CHM BLOG](https://computerhistory.org/blogs/)  [From the Collection](https://computerhistory.org/blog/category/from-the-collection/)

Adobe Photoshop Source Code
===========================

#### By [Leonard J. Shustek](https://computerhistory.org/profile/leonard-j-shustek/) | February 13, 2013

![](https://computerhistory.org/wp-content/themes/bsdstarter/assets/img/basic-page-core-memory.svg)
