...
- python
- tutorial
summary: '**nanochat** is a minimal, full-stack implementation of a ChatGPT-like language
  model that can be trained and deployed for approximately $100 on a single 8-GPU
  node in about 4 hours. The project includes everything needed—tokenization, pretraining,
  finetuning, evaluation, and a web UI—in a clean, hackable codebase designed to be
  educational and accessible. While the resulting model (2.2 billion parameters) performs
  below modern LLMs and makes frequent mistakes, it emphasizes full customization
  and transparency, allowing users to build and own their own language model from
  start to finish.'
...
[![scaling laws](/karpathy/nanochat/raw/master/dev/scaling_laws_jan26.png)](/karpathy/nanochat/blob/master/dev/scaling_laws_jan26.png)
...
Presently, the main focus of development is on tuning the pretraining stage, which takes the most amount of compute. Inspired by the modded-nanogpt repo and to incentivise progress and community collaboration, nanochat maintains a leaderboard for a "GPT-2 speedrun", which is the wall-clock time required to train a nanochat model to GPT-2 grade capability, as measured by the DCLM CORE score. The [runs/speedrun.sh](/karpathy/nanochat/blob/master/runs/speedrun.sh) script always reflects the reference way to train GPT-2 grade model and talk to it. The current leaderboard looks as follows:

| # | time | val\_bpb | CORE | Description | Date | Commit | Contributors |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
See [dev/LEADERBOARD.md](/karpathy/nanochat/blob/master/dev/LEADERBOARD.md) for more docs on how to interpret and contribute to the leaderboard.
...
The most fun you can have is to train your own GPT-2 and talk to it. The entire pipeline to do so is contained in the single file [runs/speedrun.sh](/karpathy/nanochat/blob/master/runs/speedrun.sh), which is designed to be run on an 8XH100 GPU node. Boot up a new 8XH100 GPU box from your favorite provider (e.g. I use and like [Lambda](https://lambda.ai/service/gpu-cloud)), and kick off the training script:
...
[![image](https://private-user-images.githubusercontent.com/241138/500544377-ed39ddf8-2370-437a-bedc-0f39781e76b5.png?jwt=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3ODAyNDc3ODksIm5iZiI6MTc4MDI0NzQ4OSwicGF0aCI6Ii8yNDExMzgvNTAwNTQ0Mzc3LWVkMzlkZGY4LTIzNzAtNDM3YS1iZWRjLTBmMzk3ODFlNzZiNS5wbmc_.SoTgZWW6JngRHzE3zVscVyOnVyvG2UyBabwT5ZvraeI)](https://private-user-images.githubusercontent.com/241138/500544377-ed39ddf8-2370-437a-bedc-0f39781e76b5.png?jwt=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJnaXRodWIuY29tIiwiYXVkIjoicmF3LmdpdGh1YnVzZXJjb250ZW50LmNvbSIsImtleSI6ImtleTUiLCJleHAiOjE3ODAyNDc3ODksIm5iZiI6MTc4MDI0NzQ4OSwicGF0aCI6Ii8yNDExMzgvNTAwNTQ0Mzc3LWVkMzlkZGY4LTIzNzAtNDM3YS1iZWRjLTBmMzk3ODFlNzZiNS5wbmc_.SoTgZWW6JngRHzE3zVscVyOnVyvG2UyBabwT5ZvraeI)
...
If you are a researcher and wish to help improve nanochat, two scripts of interest are [runs/scaling\_laws.sh](/karpathy/nanochat/blob/master/runs/scaling_laws.sh) and [runs/miniseries.sh](/karpathy/nanochat/blob/master/runs/miniseries.sh). See [Jan 7 miniseries v1](https://github.com/karpathy/nanochat/discussions/420) for related documentation. For quick experimentation (~5 min pretraining runs) my favorite scale is to train a 12-layer model (GPT-1 sized), e.g. like this:
