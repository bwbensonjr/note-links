...
- ai
- tutorial
summary: 'Self-hosting has become practical and fun in 2026 thanks to three convergent
  technologies: affordable mini PCs (under $500), simple networking via Tailscale,
  and AI CLI agents like Claude Code that handle infrastructure complexity. The author
  set up a home server running services like Vaultwarden, Immich, and others to replace
  SaaS providers, using Claude Code to automate Docker configuration and maintenance
  tasks that previously required extensive manual setup.


  This approach is now accessible to software-literate people who want control over
  their data without becoming sysadmins, making self-hosting genuinely enjoyable rather
  than burdensome for the first time.'
...
by [Jordan Fulghum](https://fulghum.io), January 2026
...
That changed recently, because CLI agents like [Claude Code](https://claude.com/claude-code) make self-hosting on a cheapo home server dramatically easier and actually fun.
...
|  |  |
| --- | --- |
| Cheap, capable mini PCs | You can buy a silent, low-power Linux box for less than $500 |
| Tailscale | Simple, secure, private networking |
| Claude Code | Install the CLI agent *directly on the box*, and you no longer need to remember or look-up Docker syntax, Compose quirks, or config formats |

So instead of Googling things like “docker compose vaultwarden caddy reverse proxy” and piecing together a bunch of old blog posts, *I just ran Claude Code directly on the machine* and let it sort it out.
...
I previously ran my Plex server on an M1 Mac mini, which was great, but as I wanted to add more services I found myself running a lot of resource-hungry VMs (via UTM) and it was getting complicated anytime the Mac rebooted. So, I picked up a [Beelink Mini N150](https://www.bee-link.com/products/beelink-me-mini-n150). It is small, quiet, and just barely sips power. I paid around $379\* for the device and another few hundred USD for 8TB in NVMe SSD. It's pretty wild how accessible these mini PCs have become in recent years!
...
![Claude Code running on the Linux server via SSH](claude-code.png)
...
| --- | --- |
| [Vaultwarden](https://github.com/dani-garcia/vaultwarden) | Bitwarden, but self-hosted and fast |
| [Plex](https://plex.tv) | Media server - PSA get Plex Pass to unlock hardware-accelerated transcoding |
...
| [Readeck](https://readeck.org/en/) | Read-it-later. Honestly blown away by this one! |

Each one lives in its own container.

I can access everything from my phone, laptop, and tablet like it is local.

![Uptime Kuma dashboard showing service monitoring](uptime-kuma.png)

Uptime Kuma keeping an eye on everything.

![Email inbox showing service up/down alerts](monitor.png)
...
![Immich photo timeline interface](immich.png)
...
![Readeck interface showing saved articles](read-deck.png)
...
I took a bet on [Readeck](https://readeck.org/en/). The UI is genuinely good. Clean typography, nice reading experience, good mobile support. It always remembers where I stopped reading and takes me right there. I even set up a shortcut that allows me to save an article for later right from mobile Firefox. Awesome.
...
Follow me on [Twitter](https://x.com/jordanful) for more.
