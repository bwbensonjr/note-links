...
domain: armaansood.com
source_date: '2026-03-23'
tags:
- distributed-systems
- database
- tutorial
summary: The author describes their five-year experience running a systems reading
  group at Microsoft, which evolved from focusing on database internals to covering
  broader systems topics like datacenters and distributed systems. Key lessons include
  maintaining consistency over ambition, allowing the group's scope to grow organically,
  using guided series instead of one-off papers, embracing collaborative learning,
  and making attendance accessible for those unprepared. Beyond technical knowledge,
  the author found value in building connections across the company with like-minded
  engineers and researchers interested in systems topics.
...
The more database papers we read, the more we found ourselves pulling on threads that led outside of databases. A storage engine paper would turn into a conversation about memory hierarchies. A replication paper would lead us into consensus protocols. Over time we started deliberately reading papers on adjacent topics, like [What Every Programmer Should Know About Memory](https://people.freebsd.org/~lstewart/articles/cpumemory.pdf) and [Paxos Made Simple](https://lamport.azurewebsites.net/pubs/paxos-simple.pdf).

In 2024, we shifted from one-off papers to guided reading series. We worked through sections of the [*Red Book*](http://www.redbook.io/) (Stonebraker and Hellerstein's *Readings in Database Systems*) over several sessions. That structure made a big difference. Instead of context-switching between unrelated papers every meeting, we could build on previous discussions and go deeper.

We've also experimented with inviting guest presenters. [Niv Dayan](https://scholar.google.com/citations?user=16LSwPEAAAAJ&hl=en), a researcher I'd worked with in the past, led a session on [Diva](https://www.vldb.org/pvldb/vol18/p3923-eslami.pdf) (before it won Best Paper at VLDB 2025!). Having the author in the room completely changed the quality of the discussion.

By 2025, the scope had grown well beyond databases, and I renamed the group to "Microsoft Systems Reading Group" to reflect that. The 2026 theme is datacenter foundations, and we will be reading through [The Datacenter as a Computer](https://link.springer.com/book/10.1007/978-3-031-01761-2) and learn about things like servers, racks, network clusters, load balancing, power supplies, cooling, efficiency, failures, etc. Things we rely on and take for granted when we build a distributed database on a public cloud.
...
If you're a Microsoft employee and want to join, you can find us at [aka.ms/msrg](https://aka.ms/msrg).
