...
title: Claude Code On-The-Go - Granda | Ideas & Code
domain: granda.org
...
- tutorial
- ai
summary: The author has built a system to run Claude Code agents on their iOS phone
  using a remote Vultr VM accessed via Termius and mosh, eliminating the need for
  a laptop or desktop. The setup uses Tailscale for secure private network access,
  tmux for session persistence, and push notifications via webhooks to alert the user
  when Claude needs input, enabling asynchronous development from anywhere. This infrastructure
  allows parallel development on multiple features simultaneously while keeping costs
  low (~$7/day) and risks contained to a disposable development environment.
