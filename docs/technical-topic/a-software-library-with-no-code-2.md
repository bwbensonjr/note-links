...
- ai
- academic-paper
summary: 'This article discusses a unique experiment: a software library called "whenwords"
  that contains no actual code, but rather consists only of specifications and tests
  that can be used by AI coding assistants to generate implementations in various
  programming languages. The author uses this concept to explore when traditional
  coded libraries might still be necessary versus when specification-only libraries
  could suffice, identifying five key scenarios where traditional code libraries remain
  valuable: performance-critical applications, complex testing requirements, need
  for support and bug fixes, importance of updates, and community/interoperability
  needs. The piece serves as a thought experiment about how software development might
  evolve as AI coding capabilities advance.'
