---
id: 1316
url: https://tyoverby.com/posts/version-control-for-everything-else/
title: Version control for everything - Ty Overby
domain: tyoverby.com
...
- devops
- github-repo
summary: The author argues that version control systems are essential for enabling
  AI agents to work effectively beyond programming tasks, using the example of managing
  scattered software development information across GitHub, Google Docs, email, and
  Slack. While building a proxy layer to stage changes across services is impractical,
  consolidating all project artifacts—issues, documentation, reviews, and code—into
  a single version-controlled repository would provide the atomic transactions, auditability,
  and parallel development capabilities that make AI assistance safe and practical.
  This infrastructure improvement would benefit not just LLMs but all developers by
  enabling branching, version history, and atomic changes across the entire development
  workflow.
...
* It would be near-impossible to track the changes that the LLM made. 
...
* The LLM could put the codebase in a bad state and you’d have no way of reverting
  ...
  This is true even if the LLM is incredibly smart and didn’t make any “mistakes” -
...
2. “Revert” would probably be out of reach. Underlying services might not provide functionality
   necessary to implement “undo"

   especially when reverting a change that has already
   had other changes stacked on top of it
   .
...
4. There’s no atomicity. If someone clicks the “publish” button and one service rejects the change
   ...
   This is especially troubling if the already-published changes can’t be
   reverted
5. The layer on top of the underlying services prevents you from seeing what the whole state of the
   ...
   review, but could only look at the diff instead of being given the diff *and* the ability to see
   the entire contents of the codebase before and after the diff is applied.
