...
397](/login?next=%2Fblog%2Fcontinuous_batching)  
...
* [![](https://cdn-avatars.huggingface.co/v1/production/uploads/5e48005437cb5b49818287a5/4uCXGGui-9QifAT4qelxU.png)](/lvwerra "lvwerra")
* [![](/avatars/46a4b6a1be7afa80920a2fcb6f6bd1c3.svg)](/wooihen "wooihen")
* [![](https://cdn-avatars.huggingface.co/v1/production/uploads/1594214747713-5e9ecfc04957053f60648a3e.png)](/lhoestq "lhoestq")
* [![](https://cdn-avatars.huggingface.co/v1/production/uploads/5f0988ad19cb630495b8147a/W9PMu6cURwe_RkwovKjdR.jpeg)](/ucalyptus "ucalyptus")
 * +391

[![Rémi Ouazan Reboul's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/6123945a0ed258ebc83f3d56/8wMHFQHEV24G_ljl4kPxQ.jpeg)](/ror) 
...
[![Luc Georges's avatar](https://cdn-avatars.huggingface.co/v1/production/uploads/1666977434736-617bc8d1000dbbbf7c225eed.png)](/mcpotato) 
...
[![Title card](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/banner.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/banner.png)
...
Each token is represented inside the network with a vector of length `d` (the *hidden dimension*). Therefore, the seven incoming tokens form a tensor xxx with shape [1,7,d]\left[1, 7, d \right][1,7,d]. `1` is the number of sequences, or batch size, which is just one in our case. `7` is the sequence length, and `d` is the hidden dimension, or the size of each token representation. Going forward, we'll use nnn instead of `7` as the sequence length.

Input tensor x x x is then projected by three matrices: the query projection Wq W\_q Wq​, the key projection Wk W\_k Wk​ and the value projection Wv W\_v Wv​. This produces three tensors Q Q Q, K K K and V V V, all of shape [1,n,A] \left[1, n , A \right] [1,n,A], where A A A is the dimension of the attention head. We call them the **query, key and value states,** respectively. This is represented on the left in the figure below.

[![proj_and_mul.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/proj_and_mul.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/proj_and_mul.png)

Next, tensors Q Q Q and K K K are multiplied together to measure similarity between tokens, producing a tensor of shape [1,n,n] \left[ 1, n , n \right] [1,n,n]. This is why we say that attention has quadratic complexity in sequence length. Computing QKT QK^T QKT requires O(n2d) \mathcal{O} \left( n^2 d \right) O(n2d) operations, so the cost is a square of n n n the sequence length. It is represented on the right in the figure above.

We then apply a boolean **attention mask** to QKT QK^T QKT to control which tokens can interact, as represented in the figure below. In this figure, the attention mask is a **causal mask**, meaning each token only interacts with tokens that came before it. This follows the intuition that a cause must come before its consequence, hence the name causal mask. The attention mask is crucial because it dictates all token interactions in the network. **Set all attention mask values to False and no token will ever interact with another in the whole network.** We'll examine attention masks more closely in a few paragraphs.

[![masking_and_softmax.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/masking_and_softmax.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/masking_and_softmax.png)
...
[![attention.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/attention.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/attention.png)
...
**Why this matters:** In continuous batching, Q Q Q, K K K, and V V V can have different numbers of tokens because, as we'll see, we'll be processing different stages (prefill and decode) at the same time. To make it more general, let's say Q Q Q has shape [1,nQ,A] \left[1, n\_Q , A \right] [1,nQ​,A], K K K has shape [1,nK,A] \left[ 1, n\_K , A \right] [1,nK​,A], and V V V has shape [1,nV,A] \left[ 1, n\_V , A \right] [1,nV​,A].

The attention scores QKT QK^T QKT then have shape [1,nQ,nK] \left[ 1, n\_Q , n\_K \right] [1,nQ​,nK​], and the attention mask has the same shape since it's applied point-wise to the scores.

After applying the attention mask and row-wise softmax, we multiply by V V V. Since we're multiplying a matrix of shape [1,nQ,nK] \left[ 1, n\_Q , n\_K \right] [1,nQ​,nK​] by one of shape [1,nV,A] \left[ 1, n\_V , A \right] [1,nV​,A], the inner dimensions must match: nK=nV n\_K = n\_V nK​=nV​. This means V V V and K K K always have the same length, so we can simplify our visualizations by only showing K K K.  
Don't worry if this seems abstract: the figures will make it concrete.

Furthermore, since we know that the attention mask is applied to QKT QK^T QKT, we know they have the same shape. Instead of representing the attention scores, we will represent the attention mask in its place.
...
[![simple_attention.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/simple_attention.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/simple_attention.png)
...
[![naive_generate.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/naive_generate.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/naive_generate.png)

To compute the attention scores of the new token, we still need the key and value projections of the previous tokens. So we need to repeat the matrix multiplication of the old tokens (in grey in the figure above) with Wk W\_k Wk​ and Wv W\_v Wv​ to retrieve a result that was already computed once before. In other terms, we are wasting compute. Let's see how we can avoid that.
...
[![cant_see_me.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/cant_see_me.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/cant_see_me.png)
...
Moreover, we already computed the K K K and V V V states for the tokens "*<bos>*", … , "*ject*" during the previous forward pass: if they have been stored, we do not need to recompute them again. This is the **KV cache**: the list of key and value states created during generation. It essentially allows one to reduce the compute cost of generating token n+1 n+1 n+1 from O(n2) \mathcal{O} \left( n^2 \right) O(n2) to O(n) \mathcal{O} \left( n \right) O(n) by avoiding recomputation of key and value projections, while paying a memory cost of O(n) \mathcal{O} \left( n \right) O(n).

[![kv_cache.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/kv_cache.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/kv_cache.png)
...
Let's be a bit more specific about the cache size, because it's a good opportunity to examine the shapes present in our model. For a model with L \mathcal L L attention layers and H H H attention heads with head dimension A A A, the total cache size needed to store one token will be 2∗L∗AH 2 \*\mathcal L \* AH 2∗L∗AH with a factor of 2 2 2 to account for both K K K and V V V.  
For instance, Llama-2-7B with L=32 \mathcal{L}=32 L=32 layers, H=32 H=32 H=32 heads, and A=128 A=128 A=128 requires 2×32×128=8,192 2 \times 32 \times 128 = 8,192 2×32×128=8,192 values per token per layer. With `float16` precision, this takes 2AH×2 2AH \times 2 2AH×2 bytes =16 = 16 =16 KB in memory.
...
[![chunked prefill.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/chunked_prefill.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/chunked_prefill.png)
...
[![padding.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/padding.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/padding.png)
...
[![batched_generation.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/batched_generation.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/batched_generation.png)
...
[![dynamic_batching.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/dynamic_batching.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/dynamic_batching.png)
...
[![concatenate.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/concatenate.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/concatenate.png)
...
[![ragged_batching.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/ragged_batching.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/ragged_batching.png)
...
[![continuous_batching.png](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/continuous_batching.png)](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/blog/continuous_batching/continuous_batching.png)
...
[![](/blog/assets/moe-transformers/thumbnail.png)
...
164
 ...
 February 26, 2026](/blog/moe-transformers)

 

### Community

![](/avatars/eb2ccce59bc69345f9a4a32fb37a5bd9.svg)

 [rupanshu](/rupanshu)

       [Nov 26, 2025](#6926b21309ea436c473a25b3)
...
[![image](https://cdn-uploads.huggingface.co/production/uploads/659f915228725b8dad817742/sS2g0VXAgnX5zUDnGlYkY.png)](https://cdn-uploads.huggingface.co/production/uploads/659f915228725b8dad817742/sS2g0VXAgnX5zUDnGlYkY.png)

 

See translation

* [![](https://cdn-avatars.huggingface.co/v1/production/uploads/1674683851722-62441cb7456803e95009a08f.jpeg)](/ArthurZ "ArthurZ")
* [![](/avatars/339b70298fe44c943be24dc39ada0e52.svg)](/kylehh "kylehh")
* [![](https://cdn-avatars.huggingface.co/v1/production/uploads/noauth/VFO_-tSfESObMKW9jPv5_.png)](/liam255 "liam255")
 * 4 replies
...
![](https://cdn-avatars.huggingface.co/v1/production/uploads/1674683851722-62441cb7456803e95009a08f.jpeg)
 ...
 [ArthurZ](/ArthurZ)
  ...
  Article author     [Nov 26, 2025](#6926c8908dde771357545581)
...
![](/avatars/abab3a7115123010c728a17203eb6aa8.svg)

 [bg90](/bg90)

       [Nov 26, 2025](#6926eab1f4cfb3d76eecff97)
...
* [![](https://cdn-avatars.huggingface.co/v1/production/uploads/1666977434736-617bc8d1000dbbbf7c225eed.png)](/mcpotato "mcpotato")
* [![](/avatars/abab3a7115123010c728a17203eb6aa8.svg)](/bg90 "bg90")
 * 2 replies
...
![](https://cdn-avatars.huggingface.co/v1/production/uploads/1666977434736-617bc8d1000dbbbf7c225eed.png)

 [mcpotato](/mcpotato)

  Article author     [Nov 27, 2025](#6928096fa2412008f5adefad)
...
![](https://cdn-avatars.huggingface.co/v1/production/uploads/no-auth/7BnCVOvg9hY2EXvg3h4p6.png)

 [naman5a](/naman5a)

       [Nov 26, 2025](#692758028b9bc560603d667c)
...
![](https://cdn-avatars.huggingface.co/v1/production/uploads/63b97e5eefe99543b34c7dd6/lRnA9OerlezpA5rQo9rJO.jpeg)

 [srinivasbilla](/srinivasbilla)

       [Nov 27, 2025](#6927ad0309f9a13f42877014)
...
![](https://cdn-avatars.huggingface.co/v1/production/uploads/64345a5f4b34368fdb045ef4/NfUgcPA08_Gl40pTrlWfe.jpeg)

 [ivanfioravanti](/ivanfioravanti)

       [Nov 27, 2025](#6927ef09c60b5a933faa9926)
...
![](/avatars/a6736ae2e8afa41e71d63ef519516f17.svg)

 [Modfiededition](/Modfiededition)

       [Nov 29, 2025](#692a799ba8c7d2d503e1558e)
...
![](/avatars/95a14d21ccba58b70da6eaa65d396753.svg)

 [knef2](/knef2)

       [Nov 29, 2025](#692b091d1ed836120f26fe27)
...
![](/avatars/ed8eb5b07702fbd9ec34938c4712421a.svg)

 [retry51776](/retry51776)

       [Dec 6, 2025](#6934aac8da60512619b7cf03)
...
![](/avatars/61ccc355c66a11221454d983895d9646.svg)

 [albertmichaelj](/albertmichaelj)

       [Dec 11, 2025](#693a2a53dd7e8433acae45ac)
...
![](/avatars/3fa10fde5147bf00724e3e1453006512.svg)

 [lwtwl](/lwtwl)

       [Dec 26, 2025](#694e61cf6b6a86343c1aaffa)

Is this diagram drawn incorrectly? Should the part I highlighted in the green box be in the first two columns on the left?  
[![image](https://cdn-uploads.huggingface.co/production/uploads/627b1f26772b403388b9c5ec/z85mqlbCm7xfkhETvfk8N.png)](https://cdn-uploads.huggingface.co/production/uploads/627b1f26772b403388b9c5ec/z85mqlbCm7xfkhETvfk8N.png)

 

See translation

* [![](/avatars/35d7f8e04a873d2aca4470b5d811bb24.svg)](/akash-inferq "akash-inferq")
 * 1 reply
...
![](/avatars/35d7f8e04a873d2aca4470b5d811bb24.svg)

 [akash-inferq](/akash-inferq)

       [Mar 12](#69b2dc7b25da1b45ee44dc85)
...
![](/avatars/5bdd5b2dcae65e17cbcbef3ec4b7e03f.svg)

 [aarhi-mahanta](/aarhi-mahanta)

       [Jan 14](#6967bb71fe3c8cf21a6696a1)
...
![](/avatars/662450b21a2471de2510e44201f4627f.svg)

 [qdai1](/qdai1)

       [Feb 3](#69820c1c414b30b5a53782c0)
...
![](https://cdn-avatars.huggingface.co/v1/production/uploads/noauth/ecpx2wUchQzthZFB14Hcp.jpeg)

 [zinchse](/zinchse)

       [28 days ago](#69f78a78c79c2b39759b4c30)
...
![](/avatars/3ec6e90c1abfbedd86985ccba3a56cb5.svg)

 [007superher](/007superher)

       [17 days ago](#6a05b7cafa2d9fe338781b3e)
...
* [![](https://cdn-avatars.huggingface.co/v1/production/uploads/5f106ce5348d4c7346cd19ab/Uu08yZZlFuj3dtG4wld3n.jpeg)](/abdullah "abdullah")
* [![](https://cdn-avatars.huggingface.co/v1/production/uploads/5f4066e079c1ba4c353d0c75/d0jUSE331F40QbnXlwtOu.png)](/spate141 "spate141")
...
* [![](https://cdn-avatars.huggingface.co/v1/production/uploads/5f5006390bcb0e71543cec17/rIygtvzvx-z0AlYcFIME-.png)](/stevhliu "stevhliu")
* [![](https://cdn-avatars.huggingface.co/v1/production/uploads/1645260898596-5f93087acf95e81b6854e184.png)](/sdadas "sdadas")
 * +385
