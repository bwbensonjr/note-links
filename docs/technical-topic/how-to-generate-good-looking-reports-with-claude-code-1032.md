---
id: 1032
url: https://martinalderson.com/posts/how-to-make-great-looking-consistent-reports-with-claude-code-cowork-codex/
title: How to generate good looking reports with Claude Code, Cowork or Codex - Martin
  Alderson
...
- javascript
- web-dev
summary: The article outlines a three-step method to generate professional, brand-consistent
  reports and presentations using Claude AI and a Node.js script. The process involves
  extracting your organization's design tokens into a reference file, creating HTML
  templates for reports and slideshows that follow your brand guidelines, and building
  a markdown-to-PDF converter that automatically formats content while maintaining
  visual consistency. This approach eliminates the need for manual formatting in word
  processors and allows teams to produce polished documents quickly by simply asking
  the AI agent to convert markdown content into reports or slides.
fetch_status: success
summarizer_model: global.anthropic.claude-haiku-4-5-20251001-v1:0
---

# How to generate good looking reports with Claude Code, Cowork or Codex - Martin Alderson
...
The first step is to get your existing brand into your agent. They are *ridiculously* good at doing this. Use a prompt like this to start. For this example I'll use NASA.gov, but obviously replace this with your own organisation's site.
...
![Design system HTML reference sheet extracted from NASA website](/img/design-system-report.png)
...
![Report and slides templates using NASA brand design system](/img/design-system-templates.png)
...
You can grab my version (but I'd recommend iterating on it yourself as you'll want probably specific slide and report formats) with some sample markdown inputs - one for the report and one for the slides on [this gist](https://gist.github.com/martinalderson/66007ed797ada48ac3a7d29907eb3b24).

This then produced two pretty good looking PDF outputs - you can see them here:

* [Example report PDF](/assets/europa-report.pdf)
* [Example slides PDF](/assets/europa-slides.pdf)

The final step is to make a hint in your CLAUDE.md[[1]](#fn1) so your agent can do this (feel free to do it as a skill or plugin). I'd add something like this to my user CLAUDE.md file:
...
Now every time you want to turn something in your agent into a report or slide deck you can just ask it to 'turn it into a report' or 'turn it into slides' and you should get a great looking, consistent PDF output in your organisation's brand. I've actually found it *so much better* than trying to do this in Word or Google Docs - if you get any weird formatting problems you can just ask your agent to improve the layout instead of losing your mind trying to line things up in a word processor[[2]](#fn2).

As someone with little to no innate design skill, I'm really impressed with the quality of output this approach results in. This would have either required a designer to lay out in Figma or similar (which I can't justify for *every* document I want to make), or literally hours trying to do it myself to a far poorer standard.
...
1. CLAUDE.md is a markdown file that sits in your project root and gives coding agents like Claude Code context about your project - what it does, how to build it, conventions to follow, etc. Other agents use similar files (e.g. Codex uses AGENTS.md). [↩︎](#fnref1)
2. While I'm intentionally keeping this simple, sometimes it's best to have the agent turn markdown into HTML, tweak it there (if it is changes you don't want on your "global template") *then* output it to PDF. But for simple to moderately complicated documents the markdown approach works fine. [↩︎](#fnref2)
