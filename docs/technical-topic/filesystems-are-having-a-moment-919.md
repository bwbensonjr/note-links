...
- llm
- database
summary: The AI ecosystem is rediscovering filesystems as a critical infrastructure
  for agents, not as a replacement for databases but as a solution for persistent
  context storage and memory. Rather than context windows that constantly reset like
  an erased whiteboard, filesystems allow agents to write and retain information about
  projects, preferences, and past decisions—enabling more sophisticated and autonomous
  AI tools like Claude Code. The key innovation is treating file formats as the API
  itself, allowing different applications to interoperate through standardized markdown
  files (like CLAUDE.md and SKILL.md) without requiring centralized coordination or
...
[Gaps in your Developer journey; Can you fix it?](/notes/gaps-in-your-developer-journey)
