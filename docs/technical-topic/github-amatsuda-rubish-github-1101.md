...
title: GitHub - amatsuda/rubish · GitHub
domain: github.com
...
- ruby
- cli-tool
summary: Rubish is a Unix shell written in pure Ruby that maintains full bash compatibility
  while deeply integrating Ruby features, allowing users to seamlessly mix shell commands
  with Ruby code including blocks, iterators, and libraries. It supports advanced
  features like Ruby-style conditionals, method chaining, lambda expressions, and
  can be used as a login shell or embedded in Ruby programs through its public API.
  The project offers both traditional shell syntax and Ruby enhancements, with installation
  available via Homebrew or from source, and includes a restricted mode for safely
