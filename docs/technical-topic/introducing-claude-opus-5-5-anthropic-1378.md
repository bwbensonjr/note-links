...
- llm
- ai
summary: Anthropic has introduced Claude Opus 5.5, a new flagship model that delivers
  performance comparable to Claude Fable 5.1 while costing 40% less to operate, with
  30% faster output generation and improved safety alignment. The model excels at
  complex tasks like large-scale code migrations and agentic work, with significant
  improvements in natural communication, cost efficiency through reduced token pricing,
  and enhanced safety features verified by external evaluators. Claude Sonnet 5.5
  and Haiku 5.5 will follow in coming weeks with similar performance and efficiency
  improvements.
...
Explaining a bugSummarizing a threadExplaining a design change
...
`tf.math.cumsum` replaces the bit scan. The geometry is unrolled into `ray_targets[square, direction, step]` — an ordered walk outwards. An exclusive cumsum over the step axis counts blockers strictly before each step, so the first blocker on a ray is the single step that is blocking with nothing blocking before it. This is the vectorized form of the bit scan a bitboard engine uses. I did look at `bitwise_ops` for true 64-bit bitboards — population\_count exists, but hyperbola-quintessence needs a byte-reverse primitive TF doesn't expose, so it would have fallen back to per-direction loops anyway.
