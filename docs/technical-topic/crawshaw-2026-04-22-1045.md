...
- distributed-systems
- github-repo
summary: The author is launching exe.dev, a new cloud computing platform, because
  they believe current cloud offerings have fundamental architectural flaws that make
  it unnecessarily difficult to run and manage computing resources. Rather than being
  driven by grand principles, the motivation is simply the desire to make computers
  work better—specifically addressing problems with how VMs are isolated and priced,
  the poor performance of remote storage for SSDs, and the prohibitively expensive
  networking costs that existing clouds impose. The timing is right because the proliferation
  of AI agents will dramatically increase the amount of software being written, making
  the limitations of today's cloud abstractions an even bigger pain point than before.
