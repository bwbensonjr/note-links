...
title: 'GitHub - obround/mytorch: Automatic differentiation implemented in python,
  inspired by Pytorch (easily extensible)'
domain: github.com
...
- ai
- tutorial
summary: '**MyTorch** is a Python implementation of automatic differentiation (autograd)
  inspired by PyTorch, using NumPy for computations and featuring a graph-based reverse-mode
  autodiff architecture. The library supports computing arbitrarily high-order derivatives
  for both scalar and non-scalar tensors, with a PyTorch-compatible API that includes
  `backward()` and `grad()` functions. While designed as an educational and extensible
  project rather than a practical alternative to PyTorch, it demonstrates how automatic
  differentiation can be implemented accessibly in Python and could potentially be
  extended to support GPU computation or neural network modules.'
