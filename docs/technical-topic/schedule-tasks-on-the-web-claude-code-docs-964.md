...
title: Schedule tasks on the web - Claude Code Docs
...
- web-dev
- tutorial
summary: Claude Code allows users to schedule recurring tasks on the web that run
  automatically on Anthropic's cloud infrastructure, even when your computer is off.
  You can create scheduled tasks to automate work like reviewing pull requests, analyzing
  CI failures, or running dependency audits at frequencies ranging from hourly to
  weekly. Tasks are configured through a web form, CLI, or desktop app where you specify
  the prompt, GitHub repositories, environment variables, MCP connectors, and schedule
  timing.
...
[Reference](/docs/en/claude-code-on-the-web)[Plan in the cloud](/docs/en/ultraplan)
