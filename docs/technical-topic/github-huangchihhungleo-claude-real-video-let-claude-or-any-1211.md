...
title: 'GitHub - HUANGCHIHHUNGLeo/claude-real-video: Let Claude (or any LLM) actually
  watch a video — scene-aware, deduplicated frames + transcript, from a URL or local
  file. Runs locally, MIT. · GitHub'
domain: github.com
...
- python
- cli-tool
summary: Claude-real-video is an open-source tool that enables LLMs like Claude to
  actually understand videos by intelligently extracting scene-change-based keyframes,
  deduplicating similar images, and transcribing audio—all processed locally for privacy.
  Unlike fixed-interval sampling used by other tools, it captures only meaningful
  frames, reducing token usage while preserving visual information, and outputs frames
  and transcripts that can be fed into any LLM for analysis. The tool supports URLs
  and local files, offers optional audio transcription via Whisper, and integrates
  with Claude Code as a skill for seamless video analysis.
...
[![contact sheet example](/HUANGCHIHHUNGLeo/claude-real-video/raw/master/docs/grid_example.jpg)](/HUANGCHIHHUNGLeo/claude-real-video/blob/master/docs/grid_example.jpg)
