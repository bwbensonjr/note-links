...
- distributed-systems
- web-dev
summary: The article discusses how AI agents are transitioning from synchronous chat-based
  interactions to asynchronous background operations, which creates fundamental problems
  with HTTP-based transport that was designed for real-time streaming responses. Major
  platforms like Anthropic, ChatGPT, and Cloudflare are addressing this shift by adding
  features like scheduled tasks, webhooks, and remote control, but current solutions
  primarily focus on durable state storage rather than solving the underlying transport
  problem of reliably delivering agent results across disconnections and multiple
  users. The author argues that a proper solution requires both durable state management
  and durable transport mechanisms, similar to how OpenClaw separates agent work from
  human connection using external chat systems.
...
![Standard design around single http request](/img/http-sse-agent-transport.svg)
...
Agents are getting workflows in [Temporal](https://temporal.io/), [Vercel WDK](https://vercel.com/blog/introducing-workflow),
[Relay.app](https://www.relay.app/), etc. A human sitting at a terminal or webchat is just one mode now,
...
![Openclaw channels model](/img/openclaw-channels-model.svg)

Anthropic’s direct response to the OpenClaw model is [Channels](https://code.claude.com/docs/en/channels),
which is MCP based and allows you to push messages async from an external chat system into a Claude Code
session. But they also have [`/loop` and `/schedule` slash commands](https://code.claude.com/docs/en/scheduled-tasks),
as well as [Routines](https://code.claude.com/docs/en/routines), both allowing you to schedule and
run agents in the background. Anthropic also has [Remote
Control](https://code.claude.com/docs/en/remote-control), which lets you continue a Claude Code
session from your phone or another browser.

ChatGPT has [scheduled tasks](https://help.openai.com/en/articles/10291617-tasks-in-chatgpt) which trigger agents async, that can reach out to you if needed.

Cursor has [background agents](https://cursor.com/docs/cloud-agent) that run in the background in the cloud.
...
request and streaming the tokens back on the HTTP response as an SSE stream. I’ve said before that [a
chatbot’s worst enemy is page refresh](https://zknill.io/posts/chatbots-worst-enemy-is-page-refresh/),
...
for you. OpenClaw’s model separates the *lifetime of the agent’s work* from the *lifetime of the
...
Cloudflare are getting involved too with their own Agents platform built on their workers platform. The
[Cloudflare Sessions API](https://developers.cloudflare.com/agents/api-reference/sessions/)
provides the session and conversation storage for agents, accessible over HTTP.
And to fix the async notification problem, Cloudflare has launched their [Email for Agents](https://blog.cloudflare.com/email-for-agents/)
product.

These solutions solve only one half
-----------------------------------

The problem actually splits into two halves. The first half is *durable state*. Where does the
agent’s state live, how does the agent have access to that state on restart or when processing async tasks, and
where does the agent store its output? The second half is *durable transport*. How do the bytes of
...
building durable state storage and management for agents. Their solution to *getting* the bytes of
...
I work for [Ably](https://ably.com/ai-transport) and we are currently building a durable transport for AI agents built around the idea of a session.
...
![Ably’s transport](/img/ably-pubsub-agent-transport.svg)
...
thanks for reading. · [discuss on hn](https://news.ycombinator.com/submitlink?u=https%3A%2F%2Fzknill.io%2Fposts%2Fall-your-agents-are-going-async%2F&t=All%20your%20agents%20are%20going%20async)
[← all posts](/posts/)
