...
- ai
- llm
summary: Anthropic's Project Glasswing has discovered over 10,000 high- or critical-severity
  vulnerabilities in critical software worldwide within its first month, with AI model
  Mythos Preview identifying bugs at unprecedented rates—including 2,000 at Cloudflare
  and 271 in Firefox. The major challenge has shifted from finding vulnerabilities
  to the human capacity required to verify, disclose, and patch them, with open-source
  software maintainers becoming capacity-constrained by the volume of discoveries.
  The initiative demonstrates both the security benefits of AI-assisted vulnerability
  detection and the urgent need for scalable processes to manage the verification
  and patching bottleneck.
