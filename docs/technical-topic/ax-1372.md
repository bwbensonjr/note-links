...
- devops
- github-repo
summary: AX is Google's open-source agentic orchestrator that enables developers to
  declare and run agent tasks at massive scale through isolated sandboxes with built-in
  workspace setup, network policies, and model configuration. It treats agents as
  a new class of workload distinct from microservices or batch jobs, offering four
  core primitives (Task, Workspace, Gateway, Model) and leveraging the Agent Substrate
  runtime to support billions of concurrent tasks with sub-second suspension and resumption
  capabilities. The platform is designed for both application developers and AI researchers,
  providing ergonomic tools to build, evaluate, and scale agent applications while
  handling state management, resource isolation, and cost efficiency automatically.
...
![AX's pink axolotl mascot](./axolotl.svg)
...
[Get started](https://github.com/google/ax#quick-start)
...
Run untrusted agent code in a sandbox with CPU and memory limits. Cheap to create, suspend, and throw away.](https://github.com/google/ax/blob/main/docs/concepts.md#task)
[Workspace
...
every sandbox before the task starts.](https://github.com/google/ax/blob/main/docs/concepts.md#workspace)
[Gateway

### Network policies

Define and quickly manage network policies. Lock traffic down to an explicit allowlist of hosts and
ports, inject credentials to the incoming requests.](https://github.com/google/ax/blob/main/docs/concepts.md#gateway)
[Model
...
one apply.](https://github.com/google/ax/blob/main/docs/concepts.md#model)
...
Substrate](https://github.com/agent-substrate/substrate) but provides agentic abstractions and generative runtime components.
