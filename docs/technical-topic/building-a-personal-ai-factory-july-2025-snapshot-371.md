...
- distributed-systems
- python
summary: 'The author describes building a "personal AI factory"—a system of multiple
  AI agents (Claude, o3, Sonnet models) working in parallel to generate, verify, and
  improve code automatically. The key principle is to fix the *inputs* (plans, prompts,
  agent configurations) rather than manually patching *outputs*, so the system continuously
  improves itself and learns from mistakes across all future tasks.


  The workflow involves o3 creating plans, Sonnet models executing them, and verification
  agents checking results against the original requirements. By encoding lessons back
  into the planning system rather than making one-off code fixes, the author has created
  a self-improving system that can handle increasingly complex tasks with minimal
  manual intervention.'
...
![AI Factory](../images/posts/ai-factory-header.jpg)
