...
- llm
- python
summary: QM is an open-source multiplayer agent harness designed for startups that
  allows employees to work with AI agents both independently and collaboratively within
  Slack and web interfaces. Unlike personal assistant-style agents, QM provides isolated
  workspaces for each user with their own scoped memory, files, and permissions, while
  supporting shared collaboration in channels and projects, and works with multiple
  AI models including Claude Code and OpenCode. The platform emphasizes security by
  having agents act with user credentials and permissions, offers customizable deployment
  configurations, and allows organizations to maintain private forks of the codebase
  while keeping the core synchronized with upstream updates.
...
[![The QM web UI: two concurrent sessions, a sidebar of personal files, crons, keychain, deploys, memory, and skills](/yc-software/qm/raw/main/docs/screenshots/web-ui-hero.png)](/yc-software/qm/blob/main/docs/screenshots/web-ui-hero.png)
...
`.txt` or `.md` file in [`adrs/`](/yc-software/qm/blob/main/adrs), and if we're aligned we'll handle the
implementation. Report vulnerabilities privately — see [`SECURITY.md`](/yc-software/qm/blob/main/SECURITY.md),
not a public issue.
