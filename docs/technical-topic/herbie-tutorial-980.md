...
- cli-tool
- javascript
summary: Herbie is a tool that automatically rewrites floating-point expressions to
  improve their accuracy, addressing inherent inaccuracies in floating-point arithmetic.
  The tutorial demonstrates how to use Herbie's web interface to input expressions
  and receive alternative implementations that are more accurate, and walks through
  a practical example of debugging a complex square root function in a JavaScript
  math library by identifying the mathematical core, converting it to Herbie input
  format, and applying Herbie's suggested improvements. Key takeaways include that
  Herbie can significantly boost accuracy (in the tutorial example, from 53.2% to
  99.7%), may also improve performance, and works best when users expand intermediate
  variables and test each code branch separately.
...
[inaccurate](error.html); even 0.1 + 0.2 ≠ 0.3 in
floating-point. Herbie helps find and fix these mysterious
inaccuracies.

To get started, [download and install](installing.html)
...
![The program input field in the Herbie web UI.](web-input.png)
...
![The input range field in the Herbie web UI.](range-input.png)
...
![Statistics and error measures for this Herbie run.](report-large.png)
...
and also 1.9× faster. The [rest of the
result page](report.html) shows each of these alternatives, including
details like how they were derived. These details are all
[documented](report.html), but for the sake of the
...
they write. Herbie has [options](options.html) to log
all the expressions you enter so that you can refer to them later.

But to keep the tutorial focused, let's suppose you're instead
...
To demonstrate the workflow, let's walk through
[bug 208](https://github.com/josdejong/mathjs/pull/208)
in [math.js](http://mathjs.org), a math library for
JavaScript. The bug deals with inaccurate square roots for complex
numbers. (For a full write-up of the bug itself, check out
a [blog
post](https://pavpanchekha.com/blog/casio-mathjs.html) by one of the Herbie authors.)
...
in [`lib/function/`](https://github.com/josdejong/mathjs/tree/master/lib/function).
...
[`arithmetic/sqrt.js`](https://github.com/josdejong/mathjs/blob/da306e26ed34272db44e35f07a3b015c0155d99a/lib/function/arithmetic/sqrt.js).
...
![Restricting the input range to xim >= 0.](range-input-2.png)
This asks Herbie to consider only non-negative values
of `xim` when improving the accuracy of this expression.
...
Herbie will churn for a few seconds and produce a results page.
In this case, Herbie found 4 alternatives, and we're interested in
the most accurate one, which should have an
[accuracy of 84.6%:](error.html)

![](problematic-highlight.png)
...
Herbie also found other alternatives, which are less accurate but
might be faster. You can see a summary in this table:

![](problematic-pareto-table.png)
...
accurate alternative should be pretty similar.

That alternative itself is shown lower down on the page:

![](problematic-improved-accuracy.png)
...
If you want to see more about how Herbie derived this result, you
...
The fifth alternative suggested by Herbie is much less accurate,
but it is about twice as fast as the original program:

![](problematic-improved-speed.png)
...
In this case, we were interested in the most accurate possible
...
Note that I've left the Herbie query in a comment. As Herbie
gets better, you can re-run it on this original expression to see if
it comes up with improvements in accuracy.

By the way, for some languages, including JavaScript, you can use
...
code structure, just like here.

Next steps
----------

With this change, we've made this part of the complex square
...
Please [let us know](mailto:herbie@cs.washington.edu) if
Herbie has helped you, and check out
the [documentation](../../doc.html) to learn more about
