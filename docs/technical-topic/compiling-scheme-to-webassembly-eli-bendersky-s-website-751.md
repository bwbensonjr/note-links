...
title: Compiling Scheme to WebAssembly - Eli Bendersky's website
...
- tutorial
- github-repo
summary: Eli Bendersky describes adding a new Scheme-to-WebAssembly compiler to Bob,
  his long-running open-source project implementing the Scheme programming language.
  The project explores how to lower a complex, high-level language with features like
  closures and garbage collection to WebAssembly, leveraging the WASM GC extension
  to represent Scheme objects as boxed references that the runtime manages automatically.
  Key implementation challenges included representing strings through linear memory
  offsets and implementing the `write` function directly in WASM text to handle recursive
  data structures, ultimately resulting in a well-documented 1000+ line compiler.
fetch_status: success
summarizer_model: global.anthropic.claude-haiku-4-5-20251001-v1:0
---

# Compiling Scheme to WebAssembly - Eli Bendersky's website

One of my oldest open-source projects - [Bob](https://github.com/eliben/bobscheme)
- has [celebrated 15 a couple of months ago](https://eli.thegreenplace.net/2010/11/06/bob-a-scheme-interpreter-compiler-and-vm-in-python).
...
Several months later I [added a C++ VM to Bob](https://eli.thegreenplace.net/2011/04/09/a-c-vm-added-to-bob),
...
1. Experiment with lowering a real, high-level language like Scheme to
   WebAssembly. Experiments like the recent [Let's Build a Compiler](https://eli.thegreenplace.net/2025/revisiting-lets-build-a-compiler/)
...
2. Get some hands-on experience with the WASM GC extension [[1]](#footnote-1). I have several
   samples of using WASM GC in the [wasm-wat-samples repository](https://github.com/eliben/wasm-wat-samples),
...
The new part is the rightmost vertical path. A [WasmCompiler](https://github.com/eliben/bobscheme/blob/main/bob/wasmcompiler.py)
class lowers parsed Scheme expressions all the way down to WebAssembly text,
which can then be compiled to a binary and executed using standard WASM tools [[2]](#footnote-2).
...
The compiler emits the symbols "foo" and "bar" into linear memory as follows [[3]](#footnote-3):
...
1. Deferring this to the host is difficult because the host environment has
   no access to WASM GC references - they are completely opaque.
2. Implementing it in another language (maybe C?) and lowering to WASM is also
...
Though emitting integers [directly from WASM isn't hard](https://eli.thegreenplace.net/2023/itoa-integer-to-string-in-webassembly/),
I figured this project already has enough code and some host help here would
be welcome. For all the rest, only the lowest level write\_char is used.
...
emission to WASM. Feel free to check out the source code of [WasmCompiler](https://github.com/eliben/bobscheme/blob/main/bob/wasmcompiler.py) - it's
very well documented. While it's a bit over 1000 LOC in total [[4]](#footnote-4), more than half
...
|  |  |
| --- | --- |
| [[1]](#footnote-reference-1) | The GC proposal [is documented here](https://github.com/WebAssembly/gc). It was officially added to the WASM spec in Oct 2023. |

|  |  |
| --- | --- |
| [[2]](#footnote-reference-2) | In Bob this is currently done with bytecodealliance/wasm-tools for the text-to-binary conversion and Node.js for the execution environment, but this can change in the future.  I actually wanted to use Python bindings to wasmtime, but these don't appear to support WASM GC yet. |

|  |  |
| --- | --- |
| [[3]](#footnote-reference-3) | 2048 is just an arbitrary offset the compiler uses as the beginning of the section for symbols in memory. We could also use the multiple memories feature of WASM and dedicate a separate linear memory just for symbols. |

|  |  |
| --- | --- |
| [[4]](#footnote-reference-4) | To be clear, this is just the WASM compiler class; it uses the Expr representation of Scheme that is created by Bob's parser (and lexer); the code of these other components is shared among all Bob implementations and isn't counted here. |
