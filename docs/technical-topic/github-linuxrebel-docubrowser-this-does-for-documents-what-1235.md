...
- llm
- python
summary: DocuBrowser is a local AI-powered document search engine that converts scattered
  files into a searchable knowledge base by combining keyword search with semantic
  understanding. It works entirely offline on your machine using local AI models (Ollama),
  supports dozens of file formats including PDFs, Word docs, and ebooks, and can generate
  AI-powered summaries of documents while protecting sensitive information. The tool
  features a user-friendly interface with dual search modes, tag filtering, and respects
  privacy by keeping all data local with no internet connection or API keys required.
