...
- llm
- tutorial
summary: Agent Skills are task-specific capabilities that extend Codex with packaged
  instructions, resources, and scripts to help it follow workflows reliably. Skills
  can be created, shared across teams, and invoked either explicitly through prompts
  or implicitly when Codex recognizes a matching task, with flexible storage locations
  ranging from repository-level to system-wide scopes. Users can create custom skills
  using the built-in $skill-creator tool or install pre-built skills from a curated
  GitHub collection using $skill-installer.
