...
title: Bring Your Agent to Teams | Teams SDK
...
- tutorial
- llm
summary: The Teams SDK provides a straightforward HTTP server adapter pattern that
  allows you to integrate existing agents—whether from Slack, LangChain, or Azure
  AI Foundry—into Microsoft Teams without rebuilding them. By wrapping your Express
  server with the SDK's adapter and registering a message handler, the SDK automatically
  adds a `/api/messages` endpoint and manages Teams authentication, allowing your
  existing agent to run unchanged while serving Teams users. The guide demonstrates
  three practical scenarios with code examples showing how to run Slack and Teams
  bots in the same process, connect a LangChain chain to Teams, and integrate an Azure
  Foundry agent.
...
See [Self-Managing Your Server](/teams-sdk/python/in-depth-guides/server/http-server) for the full Python guide.
