...
summary: Meta has developed a custom Computer Express Link (CXL) chip called Vistara
  ...
  machines, addressing both a memory shortage affecting 40% of its servers and rising
  RAM costs. The technology decouples older memory modules from their original constraints,
  enabling them to work alongside newer native memory without significant performance
  loss. This solution comes at a critical time when memory prices are expected to
  surge and shortages could persist through 2027.
