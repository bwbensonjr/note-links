...
title: 'GitHub - brainlid/langchain: Elixir implementation of a LangChain style framework
  that lets Elixir projects integrate with and leverage LLMs.'
domain: github.com
...
- llm
- ai
summary: Elixir LangChain is a framework that enables Elixir applications to integrate
  with large language models (LLMs) and AI services like OpenAI, Anthropic Claude,
  Google Gemini, and others, as well as self-hosted models. The library simplifies
  building AI-powered applications by providing components and pre-built chains that
  connect LLMs with external data sources and custom functionality, allowing developers
  to create data-aware and agentic applications. It takes a functional programming
  approach tailored to Elixir rather than mimicking the object-oriented design of
  the original JavaScript and Python LangChain libraries.
