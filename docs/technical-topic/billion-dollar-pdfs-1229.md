---
id: 1229
url: https://billiondollarpdf.com/
title: Billion Dollar PDFs
domain: billiondollarpdf.com
source_date: '2026-07-13'
tags:
- academic-paper
- ai
summary: Billion Dollar PDFs is a curated directory of influential documents that
  introduced transformative mental models and significantly impacted capital allocation
  across industries. The collection includes 89 documents spanning various categories—from
  technical papers like Bitcoin's whitepaper and the Transformer architecture to VC
  doctrine and macro memos—organized by era, category, and format. Users can browse
  these foundational works that shaped billions of dollars in economic decisions,
  with each entry providing context and links to the original documents.
fetch_status: success
summarizer_model: global.anthropic.claude-haiku-4-5-20251001-v1:0
---

# Billion Dollar PDFs

A directory · 2026

Billion Dollar PDFs
===================

A Billion-Dollar PDF is a memo, white paper, essay or deck that introduced a new mental model, and changed how billions of dollars of capital flowed.

The term is [Jeremy Giffon’s](https://x.com/jeremygiffon/status/1965535859073319334). This is an open index of the ones people keep citing — browse it below, expand any entry for the story, and follow the link to read the original.

![](/engraving.svg)

All receiptsStrictCultural(?)All erasPre-20002000–20092010–20192020–nowAll categoriesVC Doctrine (18)Technical Paper (16)Worldview (14)Short / Activist (11)Macro Memo (10)AI Essay (6)Pitch Deck (6)Sector Report (5)Crypto (3)All mediumsBookDeckEssayEssay SeriesLetterManifestoMemoOp-EdPaperPresentationReportResearchWhitepaperFeaturedNewestOldestA–Z

Grid viewList view

89 documents

Whitepaper

### Bitcoin: A Peer-to-Peer Electronic Cash System

A decentralized, trustless electronic cash system using proof-of-work to solve double-spending without intermediaries.

Satoshi Nakamoto2008

![](/engraving.svg)

Technical Paper

00expand +

Paper

### Attention Is All You Need

The Transformer architecture, based solely on attention, outperforms recurrence/convolution for sequence modeling.

Vaswani et al. (Google)2017

![](/engraving.svg)

Technical Paper

00expand +

Essay

### Situational Awareness: The Decade Ahead

AGI by ~2027 and superintelligence shortly after will trigger a trillion-dollar compute buildout and a US-China race.

Leopold Aschenbrenner2024

![](/engraving.svg)

AI Essay

00expand +

Paper

### The Pricing of Options and Corporate Liabilities

An option can be priced by constructing a continuously rebalanced hedge, yielding a closed-form value independent of the investor's risk preferences.

Fischer Black, Myron Scholes1973

![](/engraving.svg)

Technical Paper

00expand +

Essay

### Why Software Is Eating the World

Every company is becoming a software company; software firms will disrupt incumbents across every industry.

Marc Andreessen2011

![](/engraving.svg)

VC Doctrine

00expand +

Essay

### The Secret Tesla Motors Master Plan (just between you and me)

Bootstrap an EV company from the top down: build an expensive low-volume sports car, use the proceeds to fund progressively cheaper, higher-volume models, and drive the transition to sustainable energy.

Elon Musk2006

![](/engraving.svg)

Worldview

00expand +

Whitepaper

### Ethereum Whitepaper: A Next-Generation Smart Contract Platform

A Turing-complete blockchain enabling arbitrary smart contracts and decentralized applications.

Vitalik Buterin2013

![](/engraving.svg)

Technical Paper

00expand +

Memo

### The Internet Tidal Wave

The Internet is the single most important development since the IBM PC; Microsoft must reorient every product around it or be swept aside.

Bill Gates1995

![](/engraving.svg)

Worldview

00expand +

Paper

### The Anatomy of a Large-Scale Hypertextual Web Search Engine

Ranking web pages by the link structure of the web itself (PageRank) produces dramatically better search than keyword matching.

Sergey Brin, Lawrence Page1998

![](/engraving.svg)

Technical Paper

00expand +

Deck

### RIP Good Times

The 2008 crash is structural, not cyclical; cut burn, extend runway, get to cash-flow positive.

Sequoia Capital2008

![](/engraving.svg)

VC Doctrine

00expand +

Essay

### The Superinvestors of Graham-and-Doddsville

The concentrated success of value investors from a shared intellectual 'village' refutes efficient markets.

Warren Buffett1984

![](/engraving.svg)

Macro Memo

00expand +

Paper

### ImageNet Classification with Deep Convolutional Neural Networks (AlexNet)

Deep CNNs trained on GPUs shatter the ImageNet benchmark, igniting the modern deep-learning era.

Krizhevsky, Sutskever, Hinton2012

![](/engraving.svg)

Technical Paper

00expand +

showing 12 of 89
