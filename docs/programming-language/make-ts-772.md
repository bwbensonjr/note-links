...
- tutorial
- devops
summary: 'The article presents a workflow pattern called "make.ts" where developers
  write interactive, ad-hoc commands into a persistent file (typically named make.ts)
  and execute it, rather than typing commands repeatedly into the terminal or relying
  on shell history. This approach provides significant benefits for complex tasks,
  especially with multi-process applications: it allows for easier editing using a
  real text editor, enables running multiple commands with a single keystroke, encourages
  incremental improvement of commands, and simplifies process management through declarative
  concurrency. The author demonstrates the pattern with a concrete example of setting
  up and testing a distributed database cluster, showing how the workflow naturally
  evolves from simple sequential commands to more complex scripts with parallel execution
  and error handling.'
...
first, and then run the file. For me, I type stuff into `make.ts` and then run `./make.ts` in my terminal
...
*repeatedly* belongs to the build system. The surprising
...
There are many benefits relative to `Up Up Up`
workflow:
...
* If you need more than one command, you can write several commands,
  and still run them all with a single key (before `make.ts`, I was prone to constructing rather horrific
...
Specifically, I use [dax](https://github.com/dsherret/dax) library with Deno, which is excellent as a single-binary
batteries-included scripting environment (see [<3 Deno](https://matklad.github.io/2023/02/12/a-love-letter-to-deno.html)). Bun has a dax-like library in the box and is a
...
[uses your shell as a middleman](https://google.github.io/zx/configuration#shell), something I consider to be
sloppy ([explanation](https://julialang.org/blog/2012/03/shelling-out-sucks/)).
...
create some load. I
*almost* started to split my terminal up, but then figured
out I can do it the smart way.

The first step was cross-compiling the binary, uploading it to the
cloud machines, and running the cluster (using my [box](https://matklad.github.io/2026/01/20/vibecoding-2.html) from the other week):
