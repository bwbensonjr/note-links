...
- devops
- cli-tool
summary: Davit is a native macOS application that provides a graphical user interface
  for Apple's container platform, offering features like container management, image
  building, file browsing, and docker-compose support—all without using Electron or
  requiring administrator privileges. Built entirely in SwiftUI and using Apple's
  virtualization framework, it's lightweight (17 MB) and boots containers in under
  a second with minimal memory overhead compared to Docker Desktop. While it excels
  at core container workflows like running, building, and managing images, it lacks
  Docker CLI/API compatibility, making it unsuitable for scripted or API-dependent
  workflows.
