...
- tutorial
- devops
summary: The `uv` tool simplifies running Python scripts by automatically managing
  virtual environments and dependencies without manual setup. Scripts can be executed
  with `uv run`, with dependencies declared either via command-line flags (e.g., `--with`),
  inline metadata at the top of the script file, or project configuration files. Key
  features include support for Python version selection, dependency locking for reproducibility,
  shebang support for executable scripts, and alternative package index configuration.
