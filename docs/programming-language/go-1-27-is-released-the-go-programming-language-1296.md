...
- compilers
- news
summary: Go 1.27 introduces significant language enhancements including generic methods,
  improved struct literal initialization for nested fields, and generalized function
  type inference that eliminates the need for explicit type arguments in many contexts.
  The release also brings tooling improvements like new modernizers in go fix and
  package@version queries in go doc, along with performance optimizations that reduce
  small object allocation costs by up to 30% and new standard library packages for
  JSON processing, post-quantum cryptography, and UUID support.
...
**Previous article:** [Introducing the pkg.go.dev API](/blog/pkgsite-api)  
**[Blog Index](/blog/all)**
