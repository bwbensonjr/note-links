...
[Andrej Karpathy wrote a 200-line Python script that trains and runs a GPT from scratch](https://gist.github.com/karpathy/8627fe009c40f57531cb18360106ce95), with no libraries or dependencies, just pure Python. The script contains the algorithm that powers LLMs like ChatGPT.

Let's walk through it piece by piece and watch each part work. [Andrej did a walkthrough on his blog](https://karpathy.github.io/2026/02/12/microgpt/), but here I take a more visual approach, tailored for beginners.
...
2 max\_val = max(val.data for val in logits)

3 exps = [(val - max\_val).exp()
...
2 def \_\_init\_\_(self, data, children=(), local\_grads=()):
...
6 self.\_local\_grads = local\_grads

7

8 def \_\_add\_\_(self, other):
...
29 for child, lg in zip(v.\_children, v.\_local\_grads):
...
c = a \* b → data=6.0, local\_grads=(b.data=3, a.data=2)

L = c + a → data=8.0, local\_grads=(1, 1)

Topological sort: [a, b, c, L] (dependencies before dependents)

L.grad = 1 (dL/dL = 1)

Process L: children=(c, a), local\_grads=(1, 1)

Process c: children=(a, b), local\_grads=(3, 2)
...
This is the same algorithm that PyTorch's `loss.backward()` runs, operating on scalars instead of tensors.
...
1def gpt(token\_id, pos\_id, keys, values):

2 tok\_emb = state\_dict["wte"][token\_id]

3 pos\_emb = state\_dict["wpe"][pos\_id]

4 x = [t + p for t, p in zip(tok\_emb, pos\_emb)]

5 x = rmsnorm(x)

6

7 for li in range(n\_layer):

8 x\_residual = x

9 x = rmsnorm(x)

10 q = linear(x, attn\_wq)

11 k = linear(x, attn\_wk)

12 v = linear(x, attn\_wv)
...
16 for h in range(n\_head):

17 attn\_logits = [q\_h . k\_h[t] / sqrt(d)

18 for t in range(len(k\_h))]

19 attn\_weights = softmax(attn\_logits)

20 head\_out = weighted\_sum(attn\_weights, v\_h)

21 x = linear(x\_attn, attn\_wo)

22 x = [a + b for a, b in zip(x, x\_residual)]
...
26 x = linear(x, mlp\_fc1)

27 x = [xi.relu() for xi in x]

28 x = linear(x, mlp\_fc2)

29 x = [a + b for a, b in zip(x, x\_residual)]

30

31 logits = linear(x, lm\_head)
...
gpt(token\_id=4, pos\_id=1, ...) — processing "e" at position 1
...
Enter layer 0. Save x\_residual, normalize x again
...
Project concatenated head outputs through output projection (attn\_wo)
...
Final projection: multiply by lm\_head to get 27 logits (one per token)
...
6keys, values = [[] for \_ in range(n\_layer)], [...]

7losses = []

8for pos\_id in range(n):

9 token\_id = tokens[pos\_id]

10 target\_id = tokens[pos\_id + 1]

11 logits = gpt(token\_id, pos\_id, keys, values)

12 probs = softmax(logits)

13 loss\_t = -probs[target\_id].log()

14 losses.append(loss\_t)
...
24 m\_hat = m[i] / (1 - beta1 \*\* (step+1))

25 v\_hat = v[i] / (1 - beta2 \*\* (step+1))

26 p.data -= lr \* m\_hat / (v\_hat\*\*0.5 + eps)
...
2keys, values = [[] for \_ in range(n\_layer)], [...]
...
6for pos\_id in range(block\_size):

7 logits = gpt(token\_id, pos\_id, keys, values)

8 probs = softmax([l / temperature for l in logits])

9 token\_id = random.choices(

10 range(vocab\_size),
...
14 sample.append(uchars[token\_id])
