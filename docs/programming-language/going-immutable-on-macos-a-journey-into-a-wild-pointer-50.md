...
title: Going immutable on macOS | A journey into a wild pointer
domain: carette.xyz
...
- devops
- tutorial
summary: The author transitioned from Homebrew to nix-darwin to solve the problem
  of "imperative rot" on macOS, where manual package installations lead to unpredictable
  environments and broken configurations. Nix offers a declarative, functional approach
  using immutable configuration files and a lockfile system that enables perfect reproducibility
  across machines and easy rollbacks to previous system states. While Nix provides
  powerful benefits like ephemeral shells and comprehensive system customization,
  it comes with a steep learning curve and requires understanding complex concepts;
  the author recommends a hybrid approach combining nix-darwin for critical development
  tools with Homebrew for GUI applications.
...
As an example, this is the manual to write your own `flake.nix`: <https://nix-darwin.github.io/nix-darwin/manual/>.
